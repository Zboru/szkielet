...
export default function RegisterForm(props: { handleLoginClick?: Function }) {
    ...
    const handleLogin = (payload: { email: string, password: string }) => {
        httpManager.post('/api/users', payload).then(async (response) => {
            const { data } = await httpManager.post('/api/auth', payload);
    ...
    return (
        <div style={{
            ...
            <Card sx={{ maxWidth: 420, minWidth: 420 }}>
                <CardHeader title="Zarejestruj konto" sx={{ paddingBottom: 0 }} />
